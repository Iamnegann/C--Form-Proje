...
namespace prrbitirme
{
    public partial class Form2 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (!Regex.IsMatch(textBox4.Text, @"^[a-zA-ZğüşıöçĞÜŞİÖÇ\s]+$"))
            {
                MessageBox.Show("İsim sadece harflerden oluşmalıdır.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (!Regex.IsMatch(textBox3.Text, @"^[a-zA-ZğüşıöçĞÜŞİÖÇ\s]+$"))
            {
                MessageBox.Show("Soyisim sadece harflerden oluşmalıdır.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (!Regex.IsMatch(textBox6.Text, @"^\d+$"))
            {
                MessageBox.Show("Telefon numarası sadece sayılardan oluşmalıdır.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (textBox6.Text.Length < 10)
            {
                MessageBox.Show("Telefon numarası en az 10 haneli olmalıdır.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            try
            {
